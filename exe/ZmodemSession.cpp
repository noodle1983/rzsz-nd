...
static std::shared_ptr<Fsm::FiniteStateMachine> g_rz_fsm;
Fsm::FiniteStateMachine* ZmodemSession::getZmodemFsm()
{
    if (NULL == g_rz_fsm.get())
    {
        ...
        (*fsm) +=      FSM_EVT_S(Fsm::ENTRY_EVT,  &ZmodemSession::initState);
        ...
        (*fsm) +=      FSM_EVT_S(Fsm::ENTRY_EVT,  &ZmodemSession::checkFrametype);
        ...
        (*fsm) +=      FSM_EVT_S(Fsm::ENTRY_EVT,  &ZmodemSession::parseHexFrame);
        (*fsm) +=      FSM_EVT_S(NETWORK_INPUT_EVT,  &ZmodemSession::parseHexFrame);
        ...
        //(*fsm) +=      FSM_EVENT(Fsm::ENTRY_EVT,   NEW_TIMER(10 * 1000));
        (*fsm) +=      FSM_EVT_S(Fsm::ENTRY_EVT,  &ZmodemSession::parseBinFrame);
        (*fsm) +=      FSM_EVT_S(NETWORK_INPUT_EVT,  &ZmodemSession::parseBinFrame);
        (*fsm) +=      FSM_EVENT(HANDLE_FRAME_EVT,  CHANGE_STATE(HANDLE_FRAME_STATE));
        (*fsm) +=      FSM_EVENT(RESET_EVT        ,  CHANGE_STATE(IDLE_STATE));
        (*fsm) +=      FSM_EVENT(DESTROY_EVT      ,  CHANGE_STATE(DESTROY_STATE));
        (*fsm) +=      FSM_EVENT(Fsm::TIMEOUT_EVT, CHANGE_STATE(IDLE_STATE));
        ...
        //(*fsm) +=      FSM_EVENT(Fsm::ENTRY_EVT,   NEW_TIMER(10 * 1000));
        (*fsm) +=      FSM_EVT_S(Fsm::ENTRY_EVT,  &ZmodemSession::parseBin32Frame);
        (*fsm) +=      FSM_EVT_S(NETWORK_INPUT_EVT,  &ZmodemSession::parseBin32Frame);
        (*fsm) +=      FSM_EVENT(HANDLE_FRAME_EVT,  CHANGE_STATE(HANDLE_FRAME_STATE));
        (*fsm) +=      FSM_EVENT(RESET_EVT        ,  CHANGE_STATE(IDLE_STATE));
        (*fsm) +=      FSM_EVENT(DESTROY_EVT      ,  CHANGE_STATE(DESTROY_STATE));
        (*fsm) +=      FSM_EVENT(Fsm::TIMEOUT_EVT, CHANGE_STATE(IDLE_STATE));
        ...
        //(*fsm) +=      FSM_EVENT(Fsm::ENTRY_EVT,   NEW_TIMER(10 * 1000));
        (*fsm) +=      FSM_EVT_S(Fsm::ENTRY_EVT,  &ZmodemSession::handleFrame);
        (*fsm) +=      FSM_EVENT(NETWORK_INPUT_EVT,  CHANGE_STATE(CHK_FRAME_TYPE_STATE));
        ...
        (*fsm) +=      FSM_EVT_S(Fsm::ENTRY_EVT,   &ZmodemSession::sendZdata);
        (*fsm) +=      FSM_EVENT(SEND_ZDATA_EVT,   CHANGE_STATE(SEND_ZDATA_STATE));
        (*fsm) +=      FSM_EVENT(SEND_ZDATA_LATER_EVT,   CHANGE_STATE(SEND_FLOW_CTRL_STATE));
        (*fsm) +=      FSM_EVT_S(NETWORK_INPUT_EVT, &ZmodemSession::handleFlowCntl);
        (*fsm) +=      FSM_EVENT(CHECK_FRAME_TYPE_EVT,  CHANGE_STATE(CHK_FRAME_TYPE_STATE));
        (*fsm) +=      FSM_EVENT(RESET_EVT        ,  CHANGE_STATE(IDLE_STATE));
        (*fsm) +=      FSM_EVENT(DESTROY_EVT      ,  CHANGE_STATE(DESTROY_STATE));
        (*fsm) +=      FSM_EVT_S(Fsm::TIMEOUT_EVT, &ZmodemSession::onSentTimeout);
        (*fsm) +=      FSM_EVENT(Fsm::EXIT_EVT,    CANCEL_TIMER());

        (*fsm) += FSM_STATE(SEND_FLOW_CTRL_STATE);
        (*fsm) += 	   FSM_EVENT(Fsm::ENTRY_EVT,	 NEW_TIMER(10));
        (*fsm) +=      FSM_EVENT(SEND_ZDATA_EVT, CHANGE_STATE(SEND_ZDATA_STATE));
        (*fsm) +=      FSM_EVENT(SEND_ZDATA_LATER_EVT, CHANGE_STATE(SEND_FLOW_CTRL_STATE));
        (*fsm) +=      FSM_EVT_S(NETWORK_INPUT_EVT, &ZmodemSession::handleFlowCntl);
        (*fsm) +=      FSM_EVENT(CHECK_FRAME_TYPE_EVT, CHANGE_STATE(CHK_FRAME_TYPE_STATE));
        ...
        (*fsm) +=      FSM_EVT_S(Fsm::ENTRY_EVT,  &ZmodemSession::sendFileInfo);
        (*fsm) +=      FSM_EVENT(RESET_EVT        ,  CHANGE_STATE(IDLE_STATE));
        (*fsm) +=      FSM_EVENT(DESTROY_EVT      ,  CHANGE_STATE(DESTROY_STATE));
        (*fsm) +=      FSM_EVENT(NETWORK_INPUT_EVT,  CHANGE_STATE(CHK_FRAME_TYPE_STATE));
...
void ZmodemSession::output(const char* str, ...)
{
	...
	va_list ap;
	va_start(ap, str);
	int ret = vsnprintf(buffer, sizeof(buffer), str, ap);
	va_end(ap);
	term_data(frontend_->term, 0, buffer, strlen(buffer));
}
